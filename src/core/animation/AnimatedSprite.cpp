...
AnimatedSprite::AnimatedSprite( std::shared_ptr<const AnimationClip> clip, Entity entity, ECSRegistry& registry )
	: mClip( std::move( clip ) )
	, mEntity( entity )
	, mRegistry( registry )
{
	if( mClip && mClip->GetFrameCount() > 0 )
	{
		ShowFrame( 0 );
	}
}
...
{
	if( !mPlaying || !mClip || mClip->GetFrameCount() == 0 )
	{
		return;
	}
